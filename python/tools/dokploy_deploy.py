...
def main() -> int:
    ...
    environment_id = required_env("DOKPLOY_ENVIRONMENT_ID")
    coordinator_server_id = required_env("DOKPLOY_COORDINATOR_SERVER_ID")
    worker_server_id = required_env("DOKPLOY_WORKER_SERVER_ID")

    client = DokployClient(base_url=base_url, api_key=api_key)

    coordinator_compose_id = upsert_compose(
        client=client,
        environment_id=environment_id,
        server_id=coordinator_server_id,
...
def find_compose_id(client: DokployClient, *, environment_id: str, name: str) -> str | None:
    ...
    for item in items:
        ...
        item_environment_id = item.get("environmentId")

        if item_name == name and item_environment_id == environment_id:
...
def extract_items(payload: Any) -> list[Any]:
    ...
    if isinstance(payload, dict):
        for key in ("items", "results", "data", "composes"):
...
def build_worker_env() -> str:
    coordinator_url = required_env("DISPATCH_COORDINATOR_PUBLIC_URL")
